...
float3 CalculateSpecularBlinn(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	//I_specular: specColor
	//k_specular: gColorSpecular
	//I_light : the light intensity
	//R : reflection
	//V : viewDirection
	//n : shininess
	float3 halfVector = -normalize(viewDirection + gLightDirection);
	float3 angle = max(dot(halfVector,normal),0.0f);
	specColor = gColorSpecular * pow(angle,gShininess);
...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	float3 specularAngle = max(dot(reflection, viewDirection), 0.0f);
	specColor = gColorSpecular * pow(specularAngle, gShininess);
	specColor = saturate(specColor);
...
float3 CalculateNormal(float3 tangent, float3 normal, float2 texCoord)
{
	...
	if(gUseTextureNormal)
	{
		...
		float3 sampledNormal = gTextureNormal.Sample(gTextureSampler,texCoord).xyz * 2 - 1;
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	...
	float3 reflectionColor = gCubeEnvironment.Sample(gTextureSampler,reflectVec);
	float3 refractionColor = gCubeEnvironment.Sample(gTextureSampler,refractVec);
	
	return reflectionColor * gReflectionStrength + refractionColor * gRefractionStrength;
}
