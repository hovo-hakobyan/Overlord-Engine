...
void ShadowMapRenderer::UpdateMeshFilter(const SceneContext& sceneContext, MeshFilter* pMeshFilter) const
{
	...
	ShadowGeneratorType shadowGenType = pMeshFilter->HasAnimations() ? ShadowGeneratorType::Skinned : ShadowGeneratorType::Static;
	//2. Retrieve the corresponding TechniqueContext from m_GeneratorTechniqueContexts array (Static/Skinned)
	MaterialTechniqueContext techContext = m_GeneratorTechniqueContexts[static_cast<int>(shadowGenType)];
...
void ShadowMapRenderer::Begin(const SceneContext& sceneContext)
{
	...
	//1. Making sure that the ShadowMap is unbound from the pipeline as ShaderResourceView (SRV) is important, because we cannot use the same resource 
	// as a ShaderResourceView (texture resource inside a shader) and a RenderTargetView (target everything is rendered too) at the same time. 
	// In case this happens, you'll see an error in the output of visual studio - warning you that a resource is still bound as a SRV and cannot
	//  be used as an RTV.
	...
	//  your output because depending on your usage of ShaderResources, the actual slot the ShadowMap is using can be different, but you'll see 
	// a warning pop-up with the correct slot ID in that case.
	...
	XMMATRIX lightProj = XMMatrixOrthographicLH(100.f * sceneContext.aspectRatio, 100.f, 0.1f, 500.f);

	Light& dirLight = sceneContext.pLights->GetDirectionalLight();
	XMVECTOR eyePos = XMLoadFloat4(&dirLight.position);
	XMVECTOR focusPoint =eyePos + XMLoadFloat4(&dirLight.direction);
	XMMATRIX lightView = XMMatrixLookAtLH(eyePos, focusPoint, XMVECTOR{ 0.f,1.f,0.f });
...
void ShadowMapRenderer::DrawMesh(const SceneContext& sceneContext, MeshFilter* pMeshFilter, const XMFLOAT4X4& meshWorld, const std::vector<XMFLOAT4X4>& meshBones)
{
	...
	if (shadowGenType == ShadowGeneratorType::Skinned)
	{
	...
	for (const auto& submesh : pMeshFilter->GetMeshes())
	{
		...
		//- Set IndexBuffer
		pDeviceContext->IASetIndexBuffer(submesh.buffers.pIndexBuffer, DXGI_FORMAT_R32_UINT, offset);

		//- Set correct TechniqueContext on ShadowMapMaterial - use ShadowGeneratorType as ID (BaseMaterial::SetTechnique)
		m_pShadowMapGenerator->SetTechnique(static_cast<int>(shadowGenType));
...
void ShadowMapRenderer::End(const SceneContext&) const
{
	...
	m_GameContext.pGame->SetRenderTarget(nullptr); //nullptr sets it back to default
}
